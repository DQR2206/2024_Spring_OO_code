...
public class PathTest {//包含题目[3]
    ...
    public static Collection prepareData() {
        ...
        int testNum = 100000;//测试次数,可根据需求调整
    ...
    public void testExtractLoopPath() {
       //TODO:[3]
        ...
        for (int i = 0; i < oldNodes.size(); i++) {
            for (int j = i + 1; j < oldNodes.size(); j++) {
                if (Objects.equals(oldNodes.get(i), oldNodes.get(j))) {
                    fl = true;
                    break;
                }
            }
        ...
        if (fl && oldPath.isValid()) {
             ...
             */
            if (loopPath != null) {
                List<Integer> loopNodes = loopPath.getNodes();
                int i = 0;
                for (; i < oldNodes.size() - 1; i++) {
                    if (oldNodes.get(i).equals(loopNodes.get(0))) {
                        break;
                    }
                }
                for (int j = 0; j < loopNodes.size(); j++) {
                    assertEquals(oldNodes.get(i + j), loopNodes.get(j));
                }
                /*
                ensures \result.nodes[0] == \result.nodes[\result.nodes.length - 1];
                 */
                assertEquals(loopNodes.get(0), loopNodes.get(loopNodes.size() - 1));
                /*
                assignable \nothing;
                 */
                assertEquals(oldNodes, path.getNodes());
        ...
        if (forall || !oldPath.isValid()) {
            /*
            ensures \result == null;
             */
            assertEquals(null, path.extractLoopPath());
            /*
            assignable \nothing;
             */
            assertEquals(oldNodes, path.getNodes());
