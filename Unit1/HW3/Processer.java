...
public class Processer {
    ...
    public void deletezerofactor() {
        // 把輸出表達式中沒用的0刪除 識別出 0+-  +-0的形式
        char add = '+';
        char sub = '-';
        for (int i = 0; i < this.input.length(); i++) {
            if (i == 0 && this.input.length() > 1 && this.input.charAt(i) == '0'
                    && (this.input.charAt(i + 1) == add || this.input.charAt(i + 1) == sub)) {
                if (this.input.charAt(i + 1) == add) {
                    this.input = this.input.substring(i + 2);
                    i--;
                } else if (this.input.charAt(i + 1) == sub) {
                    this.input = this.input.substring(i + 1);
                    i--;
                }
            } else if (i > 0 && this.input.charAt(i) == '0'
                    && (this.input.charAt(i - 1) == add || this.input.charAt(i - 1) == sub)) {
                this.input = this.input.substring(0, i - 1) + this.input.substring(i + 1);
                i = i - 2;
            }
        }
    }
